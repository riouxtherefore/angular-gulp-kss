...
var gulp = require('gulp'),
    ...
    kssPath = 'node node_modules/kss/bin/kss-node ',
    bower = require('./bower'),
    isWatching = false;
 ...
 * CSS
 */
gulp.task('kss-tmp', shell.task(
  [kssPath + __dirname + '/src/app/ ' + __dirname + '/.tmp/styleguide/']
));

gulp.task('kss-dist', shell.task(
  [kssPath + __dirname + '/src/app/ ' + __dirname + '/dist/styleguide/']
));
 ...
 * Dist
 */
gulp.task('dist', ['vendors', 'assets', 'styles-dist', 'scripts-dist','kss-dist'], function () {
  return gulp.src('./src/app/index.html')
